...
2. **Portable no-enterprise Windows:**
   ...
   - el usuario arranca el motor desde `suite-tax-is-portable-win64-v1.18.1.zip` con `start-suite-tax-is.ps1`;
...
## Cómo
Primero comprueba salud:

```bash
curl -s http://127.0.0.1:8000/salud
```

Si hay `SUITE_IS_ENGINE_URL`, comprueba esa URL. Si responde `{"ok":true}`, continúa.
...
- Si `http://127.0.0.1:8000/salud` ya responde `{"ok":true}`, no hace nada.
- Si defines `SUITE_IS_ENGINE_URL`, usa ese motor remoto y no instala nada en local.
...
- Arranca `uvicorn engine_service.app:app` en `127.0.0.1:8000` (loopback) y espera a `/salud`. Logs en
...
- Verifica `VERSION_MATRIX.md`: plugin `1.18.1` requiere motor funcional `1.18.1` o superior compatible.
